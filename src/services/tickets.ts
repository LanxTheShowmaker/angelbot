...
export class TicketService {
  private introMessages = new Map<string, string>();
  ...
  private get components() {
    return (this.client as WingsClient).components;
  }

  private registerStaticHandlers() {
    // "Open Ticket" button on the panel.
    this.components.set("wings:ticket:open", async (i: any) => {
      if (!i.isButton()) return;
      await this.handleOpen(i);
    });

    // Category selection shown after pressing open.
    this.components.set("wings:ticket:category", async (i: any) => {
      if (!i.isStringSelectMenu()) return;
      await this.handleCategorySelect(i);
    });

    // Modal submit (customId: wings:ticket:create:<category>).
    this.components.set("wings:ticket:create", async (i: any) => {
  ...
  private registerControlHandlers() {
    this.components.set("wings:ticket:claim", async (i: any) => this.handleClaim(i));
    this.components.set("wings:ticket:close", async (i: any) => this.handleClose(i));
    this.components.set("wings:ticket:add", async (i: any) => this.handleAddUser(i));
    this.components.set("wings:ticket:remove", async (i: any) => this.handleRemoveUser(i));
    this.components.set("wings:ticket:transcript", async (i: any) => this.handleTranscript(i));
  }
  ...
  buildPanelEmbed() {
    return embeds.info(
      "Support Tickets",
      "Open a private ticket to reach staff. Only you and staff can see the channel. Use the button below to get started.",
      [
  ...
  private async handleOpen(i: any) {
  ...
  private async handleCategorySelect(i: any) {
  ...
  private async handleCreateModal(i: any) {
    const category = (i.customId.split(":")[3] as string) ?? "General";
    const description = i.fields.getTextInputValue("description");
    const guild = i.guild as Guild;
    const opener = i.member as GuildMember;
  ...
  private async findParent(guild: Guild): Promise<CategoryChannel | null> {
    const candidates = ["tickets", "open-tickets", "open tickets"];
    for (const c of guild.channels.cache.values()) {
      if (c.type !== ChannelType.GuildCategory) continue;
      if (candidates.includes(c.name.toLowerCase())) return c as CategoryChannel;
    }
    return null;
  }

  private async uniqueChannelName(guild: Guild, base: string): Promise<string> {
  ...
  private async buildOverwrites(guild: Guild, openerId: string, config: GuildConfig) {
  ...
  private buildIntroEmbed(opener: GuildMember, category: string, description: string, claimedById: string | null) {
    const fields = [
  ...
  private buildControlRow(channelId: string) {
    const mk = (id: string, label: string, style: ButtonStyle, emoji?: string) =>
      new ButtonBuilder().setCustomId(id).setLabel(label).setStyle(style).setEmoji(emoji ?? "🔧");
    return new ActionRowBuilder<ButtonBuilder>().addComponents(
      mk(`wings:ticket:claim:${channelId}`, "Claim", ButtonStyle.Success, "✅"),
      mk(`wings:ticket:close:${channelId}`, "Close", ButtonStyle.Danger, "🔒"),
      mk(`wings:ticket:add:${channelId}`, "Add User", ButtonStyle.Primary, "➕"),
      mk(`wings:ticket:remove:${channelId}`, "Remove User", ButtonStyle.Secondary, "➖"),
      mk(`wings:ticket:transcript:${channelId}`, "Transcript", ButtonStyle.Secondary, "📄"),
    );
  }

  private async getTicketChannel(guild: Guild, channelId: string): Promise<TextChannel | null> {
    const ch = guild.channels.cache.get(channelId) ?? (await guild.channels.fetch(channelId).catch(() => null));
    return (ch as TextChannel | null) ?? null;
  }

  private async editIntro(channelId: string, embed: EmbedBuilder) {
    const channel = (this.client.channels.cache.get(channelId) as TextChannel | undefined) ?? null;
    if (!channel) return;
  ...
  private parseChannelId(customId: string, prefix: string): string | null {
    const rest = customId.slice(prefix.length);
    if (!rest.startsWith(":")) return null;
    return rest.slice(1).split(":")[0] ?? null;
  }

  private async handleClaim(i: any) {
    if (!i.isButton()) return;
  ...
  private async handleClose(i: any) {
    ...
    if (i.customId.endsWith(":confirm")) {
      ...
      await this.closeTicket(i.guild, channelId, i.member as GuildMember);
  ...
  private async closeTicket(guild: Guild, channelId: string, closer: GuildMember) {
    ...
    if (ticket) {
      ...
      if (opener) await channel.permissionOverwrites.edit(opener.id, { SendMessages: false }).catch(() => {});
    }
    ...
    if (transcript) await this.deliverTranscript(guild, channelId, transcript, "closed");
  }

  private async handleAddUser(i: any) {
  ...
  private async handleRemoveUser(i: any) {
  ...
  private async handleTranscript(i: any) {
    if (!i.isButton()) return;
  ...
  private async buildTranscriptText(channel: TextChannel): Promise<string> {
    const messages = await channel.messages.fetch({ limit: 200 });
    const lines: string[] = [`Transcript for #${channel.name} (${channel.id})`, `Generated ${new Date().toISOString()}`, ""];
    for (const m of [...messages.values()].reverse()) {
      if (m.author.bot && m.embeds.length && !m.content) continue;
  ...
  private async deliverTranscript(guild: Guild, channelId: string, text: string, kind: string) {
    const file = new AttachmentBuilder(Buffer.from(text, "utf-8")).setName(`transcript-${channelId}.txt`);
    const ticket = await this.prisma.ticket.findUnique({ where: { channelId } }).catch(() => null);
    if (ticket) {
      const opener = await guild.members.fetch(ticket.openerId).catch(() => null);
      if (opener) await opener.send({ embeds: [embeds.info("Ticket transcript", `Your transcript (${kind}).`)], files: [file] }).catch(() => {});
    }
    const config = await this.settings.get(guild.id).catch(() => null);
    if (config?.modLogChannelId) {
      const ch = guild.channels.cache.get(config.modLogChannelId) as GuildTextBasedChannel | undefined;
      if (ch) await ch.send({ embeds: [embeds.moderation("Ticket transcript", `Channel <#${channelId}> · ${kind}.`)], files: [file] }).catch(() => {});
    }
  }

  private isStaffMember(member: GuildMember, config: GuildConfig | null): boolean {
